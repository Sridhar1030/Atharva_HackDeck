...
const AdminDashboard = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col items-center">
            <div className="flex flex-col md:flex-row justify-between w-full max-w-6xl p-4 space-y-8 md:space-y-0">
                {/* Pie Chart Section */}
                <div className="bg-white shadow-lg rounded-lg p-6 w-full md:w-2/3 mb-8 transition-transform transform hover:scale-100 duration-300">
                    <h3 className="text-xl font-semibold mb-4">Voting Statistics</h3>
                    <div className="h-48">
                        <Pie
                            data={pieChartData}
                ...
                {/* Admin Info Section */}
                <div className="bg-white ms-4 shadow-lg rounded-lg p-6 w-full md:w-1/3 mb-8 transition-transform transform hover:scale-100 duration-300 sxs:pr-4">
                    <div className="flex flex-col items-center mb-4">
                        ...
                        <h2 className="text-2xl font-semibold text-gray-700">{adminData.name}</h2>
                        <p className="text-lg text-gray-600">Location: {adminData.about}</p>
                    </div>
                    <h3 className="mt-4 text-xl font-semibold">Constituency:</h3>
                    <p className="mt-2 text-gray-600">{adminData.location}</p>
                </div>
            </div>

            {/* Users Who Voted Section */}
            <h1 className="text-4xl font-bold my-8 text-center text-gray-800">Users Voted</h1>
            <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-6xl mb-8 transition-transform transform hover:scale-100 duration-300">
                <h3 className="text-xl font-semibold">Users Who Voted:</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    {adminData.usersVoted.map((user) => (
                        <div key={user._id} className="flex items-center border rounded-lg p-4 bg-gray-50 shadow-md transition-transform transform hover:scale-105 duration-300">
                            <img src={user.image} alt={user.voterId} className="w-16 h-16 rounded-full mr-4" />
